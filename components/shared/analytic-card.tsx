...
export function AnalyticCard({
  ...
  return (
    <Card className={cn("flex flex-1 flex-col justify-between gap-4 p-6", className)}>
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-1">
          <span className={cn("font-display text-[30px] font-semibold leading-[38px]", toneClass)}>
            {value}
          </span>
            ...
            {breakdown.map((item) => (
              <div key={item.label} className="flex items-center gap-1.5">
                <span
                  className={cn(
                    "size-2 rounded-sm",
                    item.tone === "success" ? "bg-green-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]"
                  )}
                />
        ...
        {helperText && <p className="text-xs text-status-error">{helperText}</p>}
      </div>
    </Card>
  );
}
