...
import type { ScheduleDeviceDTO, ScheduleDTO } from '@/feat/schedule/dto';
...
export function ScheduleForm({
  ...
  const [devices, setDevices] = useState<ScheduleDeviceDTO[]>([]);
  ...
  useEffect(() => {
    ...
    scheduleClientApi
      ...
      .then((result) => {
        ...
        if (
          currentDeviceId &&
          !result.some((device) => device.id === currentDeviceId)
        ) {
          setValue('deviceId', '');
        }
      })
      .catch(() => {
        ...
        setError('deviceId', {
          type: 'server',
          message: 'Could not load devices for this room.',
        });
      })
      .finally(() => {
        if (!cancelled) {
  ...
  }, [roomId, setValue, setError, clearErrors, getValues]);
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <SelectField>
            <select
              {...register('deviceId')}
              disabled={!roomId || loadingDevices}
