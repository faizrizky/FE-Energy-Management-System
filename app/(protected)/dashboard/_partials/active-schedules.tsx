  ...
  return (
    <Card className="flex w-full flex-col items-end gap-4 p-6">
      <div className="flex w-full items-center justify-between">
        <p className="text-lg font-semibold text-emerald-500">Active schedule</p>
        <div className="flex items-center gap-2">
          <Tabs value={status} onValueChange={(v) => setStatus(v as typeof status)}>
            <TabsList>
              {STATUS_TABS.map((t) => (
                <TabsTrigger key={t.value} value={t.value} className="w-[150px]">
                  {t.label}
                </TabsTrigger>
              ))}
            </TabsList>
              ...
              <TableRow key={s.id}>
                <TableCell>
                  <div className="flex flex-col gap-0.5 py-1">
                    ...
                    <span className="text-[10px] text-slate-500">{s.roomLocation}</span>
                  </div>
                </TableCell>
                ...
                <TableCell className={s.repeat ? "text-emerald-500" : "text-status-error"}>
                  {s.repeat ? "Yes" : "No"}
                </TableCell>
              </TableRow>
            ))}
