...
export function getRoomDevicesColumns({
  ...
  return {
    checkbox: (device: RoomDeviceDTO) => (
      <Checkbox checked={isSelected(device.id)} onCheckedChange={() => onToggleSelect(device.id)} />
    ),
    ...
    component: (device: RoomDeviceDTO) => <span className="text-slate-500">{device.component}</span>,
    usage: (device: RoomDeviceDTO) => (
      <span className="text-slate-500">{formatKwh(device.totalUsage24hKwh, 0)}</span>
    ),
    interval: (device: RoomDeviceDTO) => {
      ...
      return (
        <div className="flex flex-col gap-0.5">
          ...
          />
          {isBelowMinimum && <span className="text-[10px] text-status-error">Min 15 minutes</span>}
        </div>
      );
    },
    status: (device: RoomDeviceDTO) => (
      <Switch checked={device.isPowerOn} onCheckedChange={() => onTogglePower(device)} />
    ),
    action: (device: RoomDeviceDTO) => (
      <div className="flex items-center gap-2">
        <TableActionButton icon={FileClock} aria-label={`View log for ${device.tbDeviceId}`} onClick={() => onViewLog(device)} />
        <TableActionButton icon={Trash2} tone="destructive" aria-label={`Delete ${device.tbDeviceId}`} onClick={() => onDelete(device)} />
      </div>
    ),
  };
}
